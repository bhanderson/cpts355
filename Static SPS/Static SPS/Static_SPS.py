...
# this is the static scoping stack
staticStack = [(0,{})]

scoping = "dynamic"
position = 0

def printStack(printable):
   print('---Top---')
   for elt in reversed(printable):
      print(elt)
   print('---Bottom---')
...
def definition(t):
    global scoping, position

    if scoping == 'static':
        a = [x[1] for x in staticStack if x[0] == position]
        b = [x[1] for x in staticStack]
        for x in a:
            if t in x:
                return x[t]
        for x in b:
...
def evalLoop(tokens):
    ...
    global stack, dictStack, scoping, position
